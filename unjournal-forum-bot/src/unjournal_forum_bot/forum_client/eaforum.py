...
def _build_post_comments_query(post_id: str) -> str:
    """Build comments query with inlined postId.

    The EA Forum's Vulcan/Meteor GraphQL layer ignores variables inside
    ``terms``, so we inline the postId directly into the query string.
    """
    return (
        '{ comments(input: { terms: {'
        f' postId: "{post_id}",'
        ' view: "postCommentsOld",'
        ' limit: 500'
        ' } }) { results { _id contents { html } user { slug } } } }'
    )
...
class EAForumClient:
    ...
    def __init__(
        ...
        # Bots endpoint for reads (avoids redirect for httpx user agent)
        self._graphql_endpoint = GRAPHQL_ENDPOINT_BOTS
        # Main endpoint for mutations (bots endpoint has collectionName bug)
        self._graphql_mutation_endpoint = GRAPHQL_ENDPOINT
        self._client = http_client or httpx.AsyncClient(
            timeout=30.0,
            follow_redirects=True,
            headers={"User-Agent": "UnjournalBot/1.0"},
        )
    ...
    async def _fetch_post(self, post_id: str) -> ForumPost | None:
        ...
        author_slug = ""
        if result.get("user") and result["user"].get("slug"):
            author_slug = result["user"]["slug"]
    ...
    async def get_post_comments_html(self, post_id: str) -> list[tuple[str, str]]:
        """Get (html, author_slug) tuples for all comments on a post."""
        await self.rate_limiter.acquire()

        data = await graphql_request(
            self._client,
            self._graphql_endpoint,
            _build_post_comments_query(post_id),
            auth_token=self.auth_token,
        )
        ...
        for comment in results:
            ...
            html = contents.get("html", "") if contents else ""
            author_slug = ""
            if comment.get("user") and comment["user"].get("slug"):
                author_slug = comment["user"]["slug"]
            if html:
                comments.append((html, author_slug))
        return comments
    ...
    async def create_comment(self, post_id: str, html_body: str) -> str:
        ...
        data = await graphql_request(
            self._client,
            self._graphql_mutation_endpoint,
            MUTATION_CREATE_COMMENT,
            variables=variables,
            auth_token=self.auth_token,
