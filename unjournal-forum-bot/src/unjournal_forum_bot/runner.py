...
from .dedup import already_commented, is_unjournal_post
...
class RunReport:
    ...
    def summary(self) -> str:
        lines = [
            ...
            f"Skipped (own post):    {self.skipped_own_post}",
    ...
    try:
        for i, paper in enumerate(papers, 1):
            ...
            for match in matches:
                ...
                # Skip posts authored by Unjournal-affiliated users
                if is_unjournal_post(match.post.author_slug):
                    match.skip_reason = f"Own post (author @{match.post.author_slug})"
                    report.skipped_own_post += 1
                    report.matches.append(match)
                    _log_match(match, config.dry_run)
                    continue
                ...
                try:
                    comments = await client.get_post_comments_html(match.post.id)
                    is_dup, reason = already_commented(comments, match.paper)
                    if is_dup:
