...
# EA Forum's search proxy endpoint (works without Algolia credentials)
...
async def search_posts(
    ...
    """Search for posts via the forum's search API proxy.

    The EA Forum wraps Algolia behind /api/search, accepting an array of
    query objects.  Each object has ``indexName`` and ``params`` keys.

    Returns a list of hit dicts (each contains objectID, title, slug,
    baseScore, etc.).
    """
    body = [
        {
    ...
    try:
        ...
        # Response is an array of result objects, one per query
